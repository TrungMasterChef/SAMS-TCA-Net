...
LSTM, TCN, and a Transformer encoder), each individually tuned under a common
preprocessing and \num{100}-epoch budget, while using only \result{0.35}\,M
parameters---about a quarter of those of the next-best model (ResNet-1D,
\result{96.9}\%). An ablation study shows that the adaptive
...
(\result{0.35} vs \result{1.55}\,M). Convolutional models (ResNet, InceptionTime,
FCN, 1D-CNN) form the strong group, while the Transformer, LSTM, and MLP trail
despite comparable or larger parameter budgets---underlining that capacity alone
is insufficient on this benchmark.
...
\begin{table}[t]
  ...
  \begin{tabular}{l S[table-format=2.1] S[table-format=2.1] S[table-format=2.1] S[table-format=1.3] S[table-format=1.2]}
    ...
    \midrule
    MLP            & 21.6 & 20.0 & 20.0 & 0.761 & 1.16 \\
    LSTM           & 51.0 & 50.5 & 50.5 & 0.941 & 0.56 \\
