...
classes), MSCA-Net attains a test accuracy of \result{94.9}\% and a macro-F1 of
...
parameters---less than a quarter of the next-best model and an absolute
improvement of \result{5.1} accuracy points over it. An ablation study confirms
that the SE attention, the attention-pooling head, and inter-block downsampling
each contribute to the final accuracy. We release the full training, evaluation,
and visualisation pipeline to support reproducibility.
...
\begin{table}[t]
  ...
  \begin{tabular}{l S[table-format=2.1] S[table-format=2.1] S[table-format=2.1] S[table-format=1.3] S[table-format=1.2]}
    ...
    SAMS-TCA-Net   & \underline{89.8} & \underline{89.6} & \underline{89.6} & 0.994 & 1.56 \\
    AGB-Net        & 68.6 & 68.3 & 68.3 & 0.969 & 0.12 \\
    \midrule
    \textbf{MSCA-Net (ours)} & \bfseries 94.9 & \bfseries 94.8 & \bfseries 94.8 & \bfseries 0.999 & 0.35 \\
    \bottomrule
  \end{tabular}
\end{table}
...
($-3.5$ accuracy points when removed), confirming that channel recalibration is
important when many sensors carry partially redundant information. The inter-block
temporal downsampling ($-3.1$) and the learned attention-pooling head ($-2.4$)
also contribute; downsampling additionally shortens the sequence processed by
deeper blocks, so it improves accuracy and efficiency together.
...
\begin{table}[t]
  ...
  \begin{tabular}{l S[table-format=2.1] S[table-format=2.1] S[table-format=2.1]}
    ...
    \midrule
    Full MSCA-Net              & \bfseries 93.7 & \bfseries 93.7 & {--} \\
    w/o squeeze-and-excitation & 90.2 & 90.2 & -3.5 \\
    w/o attention pooling      & 91.4 & 91.5 & -2.4 \\
    w/o downsampling           & 90.6 & 90.7 & -3.1 \\
    \bottomrule
  \end{tabular}
\end{table}
