...
test accuracy of \result{97.7}\% and a macro-F1 of \result{97.7}\%, outperforming
eight standard time-series baselines (MLP, 1D-CNN, FCN, ResNet, InceptionTime,
LSTM, TCN, and a Transformer encoder) as well as two stronger attention/graph
networks, while using only \result{0.35}\,M parameters. An ablation study shows
that the adaptive graph, SE attention, attention-pooling head, and inter-block
downsampling each contribute to the final accuracy. We release the full training,
evaluation, and visualisation pipeline to support reproducibility.
...
\begin{enumerate}
  ...
  ten competing models under an identical preprocessing and training protocol
  ...
  confusion-matrix, ROC, and t-SNE analyses of the learned representation.
\end{enumerate}
...
MSCA-G achieves the highest accuracy and macro-F1 while remaining compact.
...
\begin{table}[t]
  ...
  \begin{tabular}{l S[table-format=2.1] S[table-format=2.1] S[table-format=2.1] S[table-format=1.3] S[table-format=1.2]}
    ...
    \midrule
    \textbf{MSCA-G (ours)} & \bfseries 97.7 & \bfseries 97.7 & \bfseries 97.7 & \bfseries 1.000 & 0.35 \\
    \bottomrule
  \end{tabular}
\end{table}
...
component helps. The attention-pooling head is the largest single contributor
($-5.1$ accuracy points when removed), and the \emph{adaptive sensor graph} and
squeeze-and-excitation module each add a further $+3.5$ points, confirming that
learning the cross-sensor connectivity is as valuable as channel recalibration.
Inter-block downsampling ($-2.4$) additionally shortens the sequence processed by
deeper blocks, so it improves accuracy and efficiency together.

\begin{table}[t]
  \centering
  \caption{Ablation of MSCA-G components (test set, \num{80}-epoch budget).
  ...
  \begin{tabular}{l S[table-format=2.1] S[table-format=2.1] S[table-format=2.1]}
    ...
    \midrule
    Full MSCA-G                & \bfseries 97.3 & \bfseries 97.2 & {--} \\
...
extraction combined with a learned cross-sensor graph and lightweight attention is
more effective and far more parameter efficient than larger recurrent or
Transformer models. The adaptive graph adds \num{3.5} accuracy points for roughly
a thousand extra parameters, supporting the view that explicitly modelling
inter-sensor relations is valuable for bridge SHM. The first-order
